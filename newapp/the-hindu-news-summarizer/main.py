	...
	def get(self):
		...
		<input style="width:300px" onchange="headlines(this)" class="myButton" placeholder="Pick Date(MM/DD/YYY)" type="text" id="datepicker" /></p>
		  ...
		  function () {
			...
			<footer><div align="center">Creator: Utkarsh Kaushik</div></footer>
		</body>
		"""
	...
	def get(self):
		...
		for link in soup.findAll('div',{'class':'tpaper'}):
			try:
				...
				pass	
		links=links+"</div>Creator: Utkarsh Kaushik</div></footer></body>"
...
class Headlines(webapp2.RequestHandler):
	def get(self):
		...
		links=links+"</div><footer><div align='right'>Creator: Utkarsh Kaushik</div></footer></body>"
		self.response.write(links)
...
class Articles_Text(webapp2.RequestHandler):
	def get(self):
		...
		</div><footer><div align='center'>Creator: Utkarsh Kaushik, Source: The Hindu</div></footer></body>
		"""
		# links=links+"</div>"
		self.response.write(links)
...
class Summary(webapp2.RequestHandler):
	...
	def get(self):
		...
		if article_text!="No Body To Display":
			...
			sentences=re.split(r' *(?<=[.?!;])\s+(?=\p{Lu})* *', article_text)
		...
		<a style="width:300px" class="myButton" href='"""+hrf+"""'>Go Back To Headlines</a>
		</div><footer><div align='center'>Creator: Utkarsh Kaushik</div></footer></body>
		"""
		# links=links+"</div>"
		self.response.write(links)
