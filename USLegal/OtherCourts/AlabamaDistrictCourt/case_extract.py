...
class PartiesExtractor(CitationsExtractor):
	def get_parties(self):
		...
		for i in range(num_spans):
			if 'Plaintiff' in spans[i].text:
				plaintiff = spans[i-1].text.strip().replace('\n', ' ')

			if 'Defendant' in spans[i].text:
...
class JudgeExtractor(CitationsExtractor):
	def get_judge_name(self):
		...
		for i in spans:
			if 'JUDGE' in i.text:
				judge = i.text.strip()
				judge_lines = judge.split('\n')

				for j in range(len(judge_lines)):
					judge_lines[j] = judge_lines[j].strip()

				self.judge = ', '.join(judge_lines)
