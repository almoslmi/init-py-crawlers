...
class PartiesExtractor(CitationsExtractor):
	def get_parties(self):
		...
		for i in range(num_spans):
			if 'DISTRICT OF ARKANSAS' in spans[i].text:
				plaintiff = [spans[i+1].text.strip(), spans[i+2].text.strip()]
			
			if 'Civil' in spans[i].text:
				defendant = [spans[i+1].text.strip(), spans[i+2].text.strip()]
...
class JudgeExtractor(CitationsExtractor):
	def get_judge_name(self):
		...
		for i in divs:
			if 'JUDGE' in i.text or 'Judge' in i.text:
				print i.text
