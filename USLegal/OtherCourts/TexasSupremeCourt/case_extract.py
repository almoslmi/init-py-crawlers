...
class CaseContentExtractor:
	...
	def clean_text_of_case(self):
		...
		cleaned_content = cleaned_content.replace('JUST ICE', 'JUSTICE')
...
class PartiesExtractor(CitationsExtractor):
	def get_parties(self):
		...
		print divs[3].text
		plaintiff = divs[2].text

		temp = divs[3].text.replace(' ', '')
		if 'v.' in temp or 'V.' in temp:
			defendant = divs[4].text
		else:
			defendant = None
...
class JudgeExtractor(CitationsExtractor):
	...
	def get_chief_judge(self):
		self.chief_judge = None

		# for k, v in self.judges.items():
		# 	if 'C.J.' in v:
		# 		self.chief_judge = v[v.index('C.J.') - 1]
		# 		break

		return self.chief_judge
