...
class PartiesExtractor(CitationsExtractor):
	def get_parties(self):
		...
		for i in range(num_spans):
			if 'v.' in spans[i].text:
				if spans[i].text.startswith('v.'):
					plaintiff = spans[i-1].text.strip()
					defendant = spans[i+2].text.strip()
				else:
					plaintiff = spans[i].text.split('v.')[0].strip()
					defendant = spans[i].text.split('v.')[1].strip()
...
class JudgeExtractor(CitationsExtractor):
	def get_judge_name(self):
		...
		for i in divs:
			spans_in_div = i.findAll('span')

			if 'Judge' in i.text:
				print i.text
				self.judge = i.text.strip().replace('\n', ', ')
				break
