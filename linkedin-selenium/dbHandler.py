...
#dbc.execute('SET character_set_connection=utf8;')
    con = MySQLdb.connect(host='127.0.0.1', user='root', passwd='root', db='mukund_chari')
...
def addsedudata(profile):
    ...
    #print ('INSERT INTO ucla_student_details_2012_linkedin(CCID,First name,Last name,Location,profil count,positions count,profile links) values ("%s","%s","%s","%s","%s","%s","%s")'%tuple(profile))
    #INSERT INTO ucla_student_details_2012_linkedin(CCID,FirstName,LastName,Location,ProfileCount,positionscount,links) values ("12","ac","cd","12","ac","cd","gdfg")
    cur.execute('INSERT INTO education_details(ccid,lid,school,edu_code,degree,major,duration_start,duration_end) values (%s,%s,%s,%s,%s,%s,%s,%s)',profile)
...
def addsexpdata(exp):
    ...
    #print ('INSERT INTO ucla_student_details_2012_linkedin(CCID,First name,Last name,Location,profil count,positions count,profile links) values ("%s","%s","%s","%s","%s","%s","%s")'%tuple(profile))
    #INSERT INTO ucla_student_details_2012_linkedin(CCID,FirstName,LastName,Location,ProfileCount,positionscount,links) values ("12","ac","cd","12","ac","cd","gdfg")
    cur.execute('INSERT INTO experience_details(ccid,lid,title,company_name,company_link,duration_start,end,loc) values (%s,%s,%s,%s,%s,%s,%s,%s)',exp)
